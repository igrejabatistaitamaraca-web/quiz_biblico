...
├── sql/
│   └── schema.sql    → tabelas, constraints, RLS e seed de perguntas
...
2. Abra o **SQL Editor** do projeto e execute todo o conteúdo de `sql/schema.sql` — ele cria as tabelas `rooms`, `players`, `questions`, `answers`, as políticas de RLS e insere 5 perguntas de exemplo.
3. Em **Database → Replication**, confirme que `rooms`, `players` e `answers` estão habilitadas para Realtime (o script já tenta habilitar isso via `alter publication`).
...
7. Adicione mais perguntas inserindo linhas na tabela `questions` (ou via SQL, seguindo o mesmo formato do seed).

## 3. Narração com áudio gravado

Coloque arquivos `.mp3` em `audio/` e preencha o campo `audio_url` da pergunta correspondente na tabela `questions` (ex: `audio/pergunta1.mp3`). Se o campo estiver vazio ou o arquivo não carregar, o narrador usa automaticamente a **Speech Synthesis** do navegador — o jogo nunca trava por falta de áudio.
...
- Tipos de pergunta implementados no MVP: `multiple_choice` e `bible_open`. Os tipos `true_false`, `complete_verse`, `who_am_i` e `bonus` já estão previstos na estrutura da tabela `questions`, prontos para implementação futura.
